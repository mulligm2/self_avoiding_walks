...
Goal: Study effect of weighting straight edges/curves

Method:
    1) Generate SAW using Pivot Algorithm of length n (25*n iterations to ensure random configuration)
    2) Once a SAW is generated we have a random number of straight and curved edges
    3) Run 20*N iterations of Pivot algorithm set with an acceptance ratio (Metropolis Algorithm)

Weighting

<(-10)---------------- 0 ----------------------(10)>
More curvy                              More straight

Model should be interpretted using absolute values to denote temperature, sign change is used to implement different models
*/
...
int PivotSAWCheck(vector<int> directions,set<vector<int>> checker, int pivotpoint, int pivotradius)
{
    //cout << "Pivot\n";
     /*
    Directions List 
...
void SaveWalk(vector<vector<int>> coordinates)
{
    ...
    file.open("WeightedSAWNeg100.csv");
...
vector<int> PivotAlgorithmNoWeighting(int no_iterations)
{
    ofstream file;
    ...
    for (int i = 0; i<no_iterations;i++)
    {
        ...
        SAWCheck2 = PivotSAWCheck(directions,checker,SAW_point_choice,pivot_radius);
...
vector<vector<int>> PivotAlgorithmWeighted(vector<int> directions, int no_iterations, double weighting)
{
    int SAWCheck;
    int dS;
    double r,acc;
    ...
    for (int i = 0; i<no_iterations;i++)
    {
        ...
        /*
        Now need to include weighting which checks how many straight edges are present
        Very similar to 1D potts model with q=4 
        dS is a local change  
        */
        ...
        SAWCheck = PivotSAWCheck(directions,checker,SAW_point_choice,pivot_radius);
        //SAWCheck = 1;
        if (SAWCheck==0)
        {
            directions = directions_save;
    ...
    coordinates = GenerateWalk(directions,coordinates);
...
int main()
{
    ...
    vector<int> directions_vector(n);
    int big_number{100};
    double weighting{-5.0};
    double length;
    
    
    iterations = 10*n;

    directions_vector = PivotAlgorithmNoWeighting(iterations);
    iterations = 30*n;
    coords = PivotAlgorithmWeighted(directions_vector, iterations,weighting);
    SaveWalk(coords);
